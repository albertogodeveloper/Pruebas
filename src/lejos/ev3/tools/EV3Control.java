...
public class EV3Control implements ListSelectionListener, NXTProtocol, ConsoleViewerUI {
    ...
    private void createSleepPanel() {
        ...
        if (menu != null) {
            try {
                ...
                if (sleep >= 0 && sleep <= 10) {
                    sleepList.setSelectedIndex(sleep);
                }
            } catch (Exception e) {
        ...
        configButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    if (row < 0) {
                        showMessage("Please select an access point");
                    }
                } catch (Exception x) {
    ...
    private void createBluetoothPanel() {
        TableModel dataModel = new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int col) {
                return (col == 0 ? bluetoothDevices[row].getName() : bluetoothDevices[row].getAddress());
            }

            public String getColumnName(int col) {
                return (col == 0 ? "Name" : "Address");
            }
        ...
        pairButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (row < 0) {
                    showMessage("Please select a device from the table");
                } else {
                    showMessage("Pair not yet implemented");
                }
            }
        });
    }
        ...
        new FileDrop(null, programsTablePane, /*dragBorder,*/ new FileDrop.Listener() {
                    public void filesDropped(java.io.File[] files) {
                        for (int i = 0; i < files.length; i++) {
                            ...
                            try {
                                if (row >= 0) {
                                    fmPrograms.delete(fileName, row);
                                }
                            } catch (IOException e) {
    ...
    private void connect() {
        ...
        if (name != null && name.length() > 0) {
           System.out.println("Connecting to name: " + name);
            try {
                ...
                if (row >= 0) {
                    updateConnectionStatus(row, EV3ConnectionState.CONNECTED);
                }
                showFiles();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void playSoundFile() {
        ...
        try {
            ...
            ev3.getAudio().playSample(new File("/home/lejos/programs/" + fileName));
        } catch (Exception ioe) {
